...
def ucr_dataset_loader(dataset_name, dataset_folder,test_flag=False):
    dataset_path = os.path.join(dataset_folder, dataset_name)
    train_file_path = os.path.join(dataset_path, '{}_TRAIN.tsv'.format(dataset_name))
    # training data
    try:
        train_raw_arr = np.loadtxt(train_file_path, delimiter='\t',ndmin=2)
    except FileNotFoundError:
        print("Wrong Training set file or file path")

    train_data = train_raw_arr[:, 1:]
    train_labels = train_raw_arr[:, 0] - 1  #indexed from 0 instead of 1
    ...
    if test_flag:
        ...
        try:
            ...
            print("Wrong Testing set file or file path")

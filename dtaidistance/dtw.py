...
def pruned_distance(s1, s2, window=None,use_c=False,use_nogil=False):
    ...
    if use_nogil:
        if dtw_c is None:
            ...
            return None
        d = dtw_c.pruned_distance_nogil(s1,s2,window-1)
        return d
    if use_c:
        if dtw_c is None:
            _print_library_missing()
            return None
        d = dtw_c.pruned_distance(s1,s2,window-1)
    else:
        window = window - 1 #done so that the convention of window matches that of dtai distance function
        ...
        cost_prev = [0.0 for i in range(m)]
        ub_partials = [0.0 for i in range(m)]

        for j in range(m):
            cost[j] = np.inf
            cost_prev[j] = np.inf
            if j == 0:
                ub_partials[m - 1] = (s1[m-1] - s2[m-1])**2
            else:
                ub_partials[m - j - 1] = ub_partials[m - j] + (s1[m-j-1] - s2[m-j-1])**2
        UB = ub_partials[0]
        ...
        for i in range(m):
            ...
            for j in range(iniJ, min(m - 1, i + window) + 1, 1):
                ...
                d = (s1[i] - s2[j]) ** 2
                cost[j] = min(min(x, y), z) + d

                # Pruning Strategy
                if foundSC == False and cost[j] <= UB:
                    sc = j
                    foundSC = True
                if cost[j] > UB:
                    if j > ec:
        ...
        final_dtw = math.sqrt(cost_prev[m - 1])
        return final_dtw
...
def distance_matrix(s, dist_type = "dtw",max_dist=None, max_length_diff=None,
    ...
    if use_c and use_nogil:
        ...
        print("Compute distances in pure C")
        ...
        if parallel:
            ...
            dists = dtw_c.distance_matrix_nogil(s,dist_type=dist_type, **dist_opts)
    if use_c and not use_nogil:
        logger.info("Compute distances in Python compiled C")
        print("Compute distances in Python compiled C")
        if parallel:
            logger.info("Use parallel computation")
            ...
            logger.info("Use serial computation")
            dist_opts['block'] = block
            dists = dtw_c.distance_matrix(s,dist_type=dist_type, **dist_opts)
    if not use_c:
            ...
            for r in it_r:
                ...
                for c in it_c:
                    if abs(len(s[r]) - len(s[c])) <= max_length_diff:
                        if dist_type=="pruned":
                            dists[r,c]=pruned_distance(s[r],s[c],window=window)
                        else:

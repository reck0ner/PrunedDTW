...
def plot_warpingpaths(s1, s2, paths, path=None, filename=None, shownumbers=False):
    ...
    # ax3.set_aspect(1)
    ax3.matshow(paths[1:, 1:],cmap="Spectral_r")
    # ax3.grid(which='major', color='w', linestyle='-', linewidth=0)
    # ax3.set_axis_off()
    py, px = zip(*p)
    ax3.plot(px, py, "-", color="white")
...
def plot_warpingpaths_heatmap(s1, s2, paths, path=None, filename=None, shownumbers=False):
    """Plot the warping paths matrix.
    ...
    ax3 = fig.add_subplot(gs[1:, 1:])
    # ax3.set_aspect(1)
    cax = ax3.matshow(paths[1:, 1:],cmap="Spectral_r")

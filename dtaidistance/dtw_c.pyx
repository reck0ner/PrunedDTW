...
def pruned_distance(np.ndarray[DTYPE_t, ndim=1] s1, np.ndarray[DTYPE_t, ndim=1] s2,
             int window=0):
    ...
    # Variables to implement the pruning in the lower and upper triangular
    cdef int sc=0,ec=0,next_ec,lp
    cdef double UB = 0
    cdef bool foundSC, prunedEC
    cdef int iniJ
    #Using O(m) space instead of O(m^2)
    cost = <double *> malloc(sizeof(double)*m)
    cost_prev = <double *> malloc(sizeof(double)*(m))

    cdef double * ub_partials
    ...
    for j in range(m):
        ...
        cost_prev[j]=inf
        if j==0:
            ub_partials[m-1] = pow(s1[m-1] - s2[m-1], 2)
        else:
            ub_partials[m-j-1] = ub_partials[m-j] + pow(s1[m-j-1]-s2[m-j-1],2)
    UB = ub_partials[0]
    ...
    for i in range(m):
        ...
        prunedEC = False
        next_ec = i + window +1
        iniJ = max(0,max(i-window,sc))
        for j in range(iniJ,min(m-1,i+window)+1,1):
            if i==0 and j==0:
                cost[j] = pow(s1[0]-s2[0],2)
                foundSC = True
                continue
            ...
            cost[j] = min(min(x,y),z) + pow(s1[i]-s2[j],2)

            #Pruning Strategy
            if not foundSC and cost[j]<=UB:
                sc = j
                foundSC =True
            if cost[j] > UB:
                if j > ec:
        ...
        # for k in range(m):
        #     printf("cost[%d]=%lf ",k,cost_prev[k])
        # printf("\n")
    ...
    cdef double final_dtw = sqrt(cost_prev[m-1])
    free(cost)
    free(cost_prev)
    return final_dtw
...
cdef double pruned_distance_nogil_c(double *s1,double *s2,
                            int m,
                            int window = 0) nogil:
    '''
    Pruned DTW C Implementation
    '''
    if window == 0:
        window = m
    window = window-1 #to make the window interpretation same as dtw distance
    # cdef int m = len(s1)
    cdef double * cost
    cdef double * cost_prev
    cdef double * cost_tmp
...
def pruned_distance_nogil(double[:] s1, double[:] s2,
             int window=0):
    ...
    if isinstance(s2, (np.ndarray, np.generic)):
        if not s2.base.flags.c_contiguous:
            logger.debug("Warning: Sequence 2 passed to method distance is not C-contiguous. " +
                         "The sequence will be copied.")
            s2 = s2.copy()
    return pruned_distance_nogil_c(&s1[0],&s2[0],len(s1),window)
...
@cython.boundscheck(False) # turn off bounds-checking for entire function
@cython.wraparound(False)  # turn off negative index wrapping for entire function
def distance_matrix(cur,str dist_type="dtw", double max_dist=inf, int max_length_diff=0,
    ...
    for r in range(block[0][0], block[0][1]):
        for c in range(max(r + 1, block[1][0]), block[1][1]):
            if labs(len(cur[r]) - len(cur[c])) <= max_length_diff:
                if dist_type=="pruned":
                    dists[r, c] = pruned_distance(cur[r],cur[c],window=window)
                else:
...
def distance_matrix_nogil(cur, dist_type = "dtw",double max_dist=inf, int max_length_diff=0,
    ...
    if is_parallel:
        ...
        if dist_type=="dtw":
            ...
            pruned_distance_matrix_nogil_c(cur2, len(cur), cur2_len, &dists[0,0], window,
                                block_rb, block_re, block_cb, block_ce)
    free(cur2)
    free(cur2_len)
    return dists_py
...
cdef distance_matrix_nogil_c(double **cur, int len_cur, int* cur_len, double* output,
    ...
    #    printf("\n")
    #printf("---\n")
    # printf("In distance matrix nogil_c\n")
    cdef int r
    cdef int c
    cdef int cb
    ...
    for r in range(block_rb, block_re):
        ...
        for c in range(cb, block_ce):
            # output[len_cur*r + c] = pruned_distance_nogil_c(cur[r], cur[c], cur_len[c],window=window)
            output[len_cur*r + c] = distance_nogil_c(cur[r], cur[c], cur_len[r], cur_len[c],window=window, max_dist=max_dist,max_step=max_step, max_length_diff=max_length_diff,penalty=penalty)



cdef pruned_distance_matrix_nogil_c(double **cur, int len_cur, int* cur_len, double* output, int window=0,int block_rb=0, int block_re=0, int block_cb=0, int block_ce=0):
    cdef int r
    cdef int c
    cdef int cb
